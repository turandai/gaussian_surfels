...
class TSDFVolume:
    ...
    def __init__(self, vol_bnds, voxel_size, use_gpu=True, margin=5):
        ...
        __global__ void integrate(float * tsdf_vol,
          ...
          // float normal_x = normal_im[pixel_y*im_w+pixel_x*3+0],
          //       normal_y = normal_im[pixel_y*im_w+pixel_x*3+1],
          //       normal_z = normal_im[pixel_y*im_w+pixel_x*3+2];
          float obs_weight = weight_im[pixel_y*im_w+pixel_x];
          // printf("%.5f", obs_weight);
          // obs_weight = other_params[5];
    ...
    def integrate(self, color_im, depth_im, weight_im, cam_intr, cam_pose, obs_weight=1.):
        ...
        if self.gpu_mode:  # GPU mode: integrate voxel volume (calls CUDA kernel)
            for gpu_loop_idx in range(self._n_gpu_loops):
                self._cuda_integrate(self._tsdf_vol_gpu,
                                     ...
                                     self.cuda.InOut(weight_im.reshape(-1).astype(np.float32)),
    ...
    def get_mesh(self):
        ...
        # print(tsdf_vol.min(), tsdf_vol.max(), tsdf_vol.shape)
...
def save_tsdf(tsdf_info, save_path, voxel_size=0.005, margin=3, save_mesh=True):
    ...
    views = len(tsdf_info['depth'])
    vol_bnds = np.zeros((3, 2))

    for i in range(views):
        depth_im = tsdf_info['depth'][i]
        cam_intr = tsdf_info['intrinsic'][i]
        # cam_pose = tsdf_info['extrinsic'][i]
        cam_pose = np.linalg.inv(tsdf_info['extrinsic'][i]).T

        # print(depth_im.shape, cam_intr, cam_pose)
        # exit()
    ...
    tsdf_vol = TSDFVolume(vol_bnds, voxel_size=voxel_size, margin=margin)

    for i in range(views):
        depth_im = tsdf_info['depth'][i]
        normal_im = tsdf_info['normal'][i]
        viewDir_im = tsdf_info['viewDir'][i]
        weight_im = (normal_im * -viewDir_im).sum(0).clip(0, 1)
        weight_im = np.ones_like(weight_im)
        # print(normal_im.shape, normal_im.min(), normal_im.max(), normal_im.mean())
        # print(viewDir_im.shape, viewDir_im.min(), viewDir_im.max(), viewDir_im.mean())
        # print(weight_im.shape, weight_im.min(), weight_im.max(), weight_im.mean())
        # exit()
        cam_intr = tsdf_info['intrinsic'][i]
        # cam_pose = tsdf_info['extrinsic'][i]
        cam_pose = np.linalg.inv(tsdf_info['extrinsic'][i]).T
        ...
        tsdf_vol.integrate(color_image, depth_im, weight_im, cam_intr, cam_pose, obs_weight=1.)

    print("Saving mesh...")
    verts, faces, norms, colors = tsdf_vol.get_mesh()

    meshwrite(os.path.join(save_path), verts, faces, norms, colors)
...
if __name__ == "__main__":
    ...
    scans = [i for i in scans if 'depth.npy' in i]
